...
Menubar.File = function ( editor ) {
	...
	var option = new UI.Row();
	option.setClass('option');
	option.setTextContent('Generate GCODE');
	option.onClick(function() {

		var tab = window.open('http://shapeforge.loria.fr/slicecrafter/', '_blank')
		tab.focus();
	});
	...
	var option = new UI.Row();
	option.setClass('option');
	option.setTextContent('Try my luck');
	option.onClick(function() {

		var exporter = new THREE.STLExporter();

		var text = exporter.parse(editor.scene);

		var blob = new Blob( [ text ], { type: 'text/plain' } );

		var formData = new FormData();

		formData.append('stl', blob);

		var xhr = new XMLHttpRequest();

		formData.append( 'stl', blob );

		xhr.open( 'POST', 'https://cs.uef.fi/~ec2l/slice.php', true );

		xhr.onreadystatechange = function(response) {

			if (xhr.readyState == XMLHttpRequest.DONE) {
        		
				alert('Your gcode file is available at ' + xhr.responseText);
